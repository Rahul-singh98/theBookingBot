...
os.environ.setdefault(
    "OPENAI_API_KEY", "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")
...
def get_listings(data):
    ...
    logger.debug("Data sent to get_listings:\n", extra=data)
...
def delete_booking(booking_id, customer_id):
    ...
    if response.status_code == 200:
        ...
        logger.error("Record not found:", extra=response.json())
    else:
        logger.error("Error:", extra=response.json())
...
class GetListingsInput(BaseModel):
    data: object = Field(
        description="has two keys, 'query_params' and 'embedding_text'")
...
class CreateBookingInput(BaseModel):
    data: object = Field(
        description="has 4 keys,'listing_id' and 'customer_id', 'start_date' and 'end_date'")
...
class GetBookingsInput(BaseModel):
    customer_id: int = Field(
        description="the id for the customer whose bookings will be returned")
...
chat_history = []
NUM_OF_CHAT_MESSAGES_TO_SAVE = 100
...
def handle_agent_input(input_val):
    ...
    result = agent_executor.invoke(
        {"input": input_val, "chat_history": chat_history})
    ...
    try:
        if ('intermediate_steps' in result):
            for i in range(0, len(result["intermediate_steps"])):
                if (result["intermediate_steps"][i][0].tool == 'GetListings'):
                    ids = map(extract_listing_id, json.loads(
                        result["intermediate_steps"][i][1]['data']))
                ...
                if (result["intermediate_steps"][i][0].tool == 'GetBookings'):
                    ids = map(extract_booking_id_and_name, json.loads(
                        result["intermediate_steps"][i][1]['data']))
        ...
        logger.error("The requested index does not exist.")
    except Exception as e:
        logger.error("Error in getting information from intermediate_steps", e)
...
if __name__ == "__main__":
    while True:
        ...
        response = handle_agent_input(user_input)
        # print("Assistant: ", response)
        if response is None:
            response = dict()

        assistant = response.get('data_to_display') if response.get(
            'data_to_display') else response.get('output')

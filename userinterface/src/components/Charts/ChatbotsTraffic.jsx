...
const ChatbotsTraffic = () => {
  const [resolution, setResolution] = useState(TIME_RANGES.HOUR);
  const [ChatbotTrafficData, setChatbotTrafficData] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const results = await PrometheusAPI.getChatbotTraffic(resolution);
        ...
        setChatbotTrafficData(formattedData);
      } catch (error) {
  ...
  const activeChatbotsOptions = {
    ...
    tooltip: {
      ...
      custom: function ({ series, seriesIndex, dataPointIndex, w }) {
        const data = ChatbotTrafficData[dataPointIndex];
        return (
          '<div class="p-2">' +
  ...
  const chartSeries = [
    {
      ...
      data: ChatbotTrafficData.map((item) => ({
        x: item.x,
        y: item.y,
      })),
    },
...
export default ChatbotsTraffic;
